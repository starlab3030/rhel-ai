...
|`domain`|도메인 필드는 3개 단어를 넘지 않는 것이 좋음|지식 문서의 주제 또는 범주<br>도메인은 합성 데이터 생성 중에 교사 모델에 프롬프트를 표시하고 추가 컨텍스트를 추가|
|`seed_examples`|***qna.yaml*** 파일에는 최소 5개의 시드 예제가 필요|지식 문서의 맥락과 함께 "*질문과 답변*" 쌍이 들어 있는 필드|
|`context`|각 ***qna.yaml***에는 5개의 컨텍스트 블록이 필요<br>최대 토큰 수는 500개|지식 문서에서 정확히 가져온 정보 덩어리<br>교사 모델을 안내하는 데 도움이 되도록 표, 단락 또는 목록을 포함한 다양한 유형의 콘텐츠를 강조 표시|
